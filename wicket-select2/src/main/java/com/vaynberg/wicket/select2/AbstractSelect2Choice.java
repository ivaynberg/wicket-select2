...
abstract class AbstractSelect2Choice<T, M> extends HiddenField<M> implements IResourceListener, IHeaderContributor {
	...
	public void renderHead(IHeaderResponse response) {
		...
		response.renderOnDomReadyJavascript(JQuery.execute("$('#%s').select2(%s);", getJquerySafeMarkupId(), settings.toJson()));
	...
	protected void onInitialize() {
		...
		ajax.setData(String
				.format("function(term, page) { return { term: term, page:page, '%s':true, '%s':[window.location.protocol, '//', window.location.host, window.location.pathname].join('')}; }",
						"wicket-ajax", "wicket-ajax-baseurl"));
	...
	protected void onConfigure() {
		...
		getSettings().getAjax().setUrl(RequestCycle.get().urlFor(null, IResourceListener.INTERFACE));
	}
	...
	*/
	...
	public void onResourceRequested() {
		...
		try {
			OutputStreamWriter out = new OutputStreamWriter(webResponse.getOutputStream(), "UTF-8");
